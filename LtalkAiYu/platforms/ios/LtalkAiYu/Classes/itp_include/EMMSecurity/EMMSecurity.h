...
@property (nonatomic,strong,readonly)NSDate *tokenDate;
@property (nonatomic,strong,readonly)NSString   *multOrgNo;
@property (nonatomic,assign,readonly)BOOL    userInfoFlag;//默认获取用户信息
...
@property (nonatomic,strong,readonly)NSDictionary *currentGlobalConfig;
@property (nonatomic,strong,readonly)NSDictionary *defaultGlobalConfig;
@property (nonatomic,strong,readonly)NSArray *globalServers;
 ...
 *  @param userID   用户id
 *  @param password 密码
 *  @param multOrgNo 多组织编号
 */
- (void)loginWithUserId:(NSString *)userID password:(NSString *)password multOrgNo:(NSString *)multOrgNo;
 ...
 *  是否手动管理校验token
 *
 *  @return 如果返回YES 需要用户自己是否管理token的校验， 如果返回NO，单点组件内部处理token校验逻辑
 */
- (BOOL)willManulVerifyToken;



/**
 单点组件内部处理token失效逻辑，询问是否跳转到moa，用户可以实现该方法来处理是否跳转moa

 @return BOOL YES 跳转 NO 不跳转
 */
- (BOOL)shouldJumpInToRootApp;
